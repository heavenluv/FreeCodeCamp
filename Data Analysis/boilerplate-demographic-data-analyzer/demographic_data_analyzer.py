...
def calculate_demographic_data(print_data=True):
    ...
    allbachelors=df.loc[df['education']=='Bachelors'].shape[0]
    percentage_bachelors = round((allbachelors/totnum)*100,1)
    ...
    lower_education_rich = round((df.loc[(df['salary']=='>50K')&((df['education']!='Bachelors')&(df['education']!='Masters')&(df['education']!='Doctorate'))].shape[0]/lower_education)*100,1)
    """
    ...
    maxc=df.loc[df['salary']=='>50K','native-country'].value_counts()/df["native-country"].value_counts()

    highest_earning_country = maxc.idxmax()
    highest_earning_country_percentage = round(maxc.max()*100,1)

    # Identify the most popular occupation for those who earn >50K in India.
    topin=df.loc[(df['salary']=='>50K')&(df['native-country']=='India'),'occupation'].value_counts()
    top_IN_occupation = topin.index[0]
    ...
    if print_data:
        ...
        print(f"Percentage of rich among those who work fewest hours: {round(rich_percentage)}%")
